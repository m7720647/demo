...
class OrderModel extends BaseModel {
    ...
    function order_goods($order_id) {
        ...
        $sql = "SELECT og.rec_id, og.goods_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, " .
                ...
                "og.goods_price * og.goods_number AS subtotal, og.extension_code, g.goods_img " .
    ...
    function get_cart_goods() {
        ...
        foreach ($res as $row) {
            ...
            if ((C('show_goods_in_cart') == "2" || C('show_goods_in_cart') == "3") && $row['extension_code'] != 'package_buy') {
                $res = $this->row("SELECT `goods_img` FROM " . $this->pre . "goods WHERE `goods_id`='{$row['goods_id']}'");
...
//    }
    ...
    function get_order_thumb($order_id) {
        ...
        $arr = $this->model->query("SELECT g.goods_img FROM " . $this->model->pre . "order_goods as og left join " . $this->model->pre . "goods g on og.goods_id = g.goods_id WHERE og.order_id = " . $order_id . " limit 1");
        return $arr[0]['goods_img'];
    }
