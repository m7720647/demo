...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($brand > 0)
    {
		...
		$brandstr = getBrandContent($brand);
		if(!empty($brandstr))
		{
			$smarty->assign('brand_name',     $brand_name);
			$smarty->assign('brand_logo',     htmlspecialchars($brandstr['brand_logo']));
			$smarty->assign('brand_content',     htmlspecialchars($brandstr['brand_desc']));
		}
    }
    else
    {
	...
	$smarty->assign('categories2',      get_categories_tree($cat_id)); // 分类树
    ...
    $goodslist = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order, $cat_id);
    if($display == 'grid')
    {
...
{
	return $GLOBALS['db']->getRow('SELECT brand_logo, brand_desc FROM ' . $GLOBALS['ecs']->table('brand') . " WHERE brand_id = '$brand_id'");
}
 ...
 * @return  array
 */
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $cat_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        if ($row['promote_price'] > 0)
        {
			...
			$half_price = $promote_price / 2;
        }
        else
        {
            $promote_price = 0;
			$half_price = $row['shop_price'] / 2;
        }
        ...
        if($cat_id != 4) {
		$arr[$row['goods_id']]['half_price']       = price_format($half_price);
		}
    }
