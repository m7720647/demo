...
function insert_cart_info()
{
    ...
    {		
		...
		if ($number != 0) $str = '<em></em>购物车( '.sprintf($GLOBALS['_LANG']['cart_info'], $number, price_format($amount, false)).' )';
		else $str = '<em></em>购物车( 0 )';
    }
    else
    {
        $number = 0;
 ...
 * @return  string
 */
function insert_cart_info_product()
{
	$str = '';
    $sql = 'SELECT c.rec_id, c.goods_id, c.goods_name, c.goods_price, c.goods_number, g.goods_thumb' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') . ' c left join ' . $GLOBALS['ecs']->table('goods') . ' as g on c.goods_id=g.goods_id' .
           " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
    ...
    if (empty($res)) {
		...
		$str .= '<div class="smc"><ul id="mcart-sigle">';
		$i = 0;
		foreach ($res AS $row)
		{
			if($i > 0) $cc = 'cLine';
			$str .= '<li class="'.$cc.'"><div class="p-img f_l"><a href="goods.php?id='.$row['goods_id'].'"><img src="'.$row['goods_thumb'].'" width="50" height="50" alt=""></a></div><div class="p-name f_l"><a href="goods.php?id='.$row['goods_id'].'" title="'.$row['goods_name'].'">'.$row['goods_name'].'</a></div><div class="p-detail"><span class="p-price"><strong>￥'.$row['goods_price'].'</strong>×'.$row['goods_number'].'</span><a href="javascript:if (confirm(\'您确实要把该商品移出购物车吗？\')) location.href=\'flow.php?step=drop_goods&id='.$row['rec_id'].'\';" title="删除">x</a></div></li>';
			$i += 1;
		}
		$str .= '</ul></div>';
		$str .= insert_cart_total_info();
	}
 ...
 * @return  string
 */
function insert_cart_total_info()
{
    $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') .
           " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
    ...
    $str = '<div class="smb"><div class="p-total">共'.$number.'件商品<br/><strong><em>'.$amount.'</em>元</strong></div><a id="btn-payforgoods" title="去购物车" href="flow.php">去购物车结算</a></div>';
